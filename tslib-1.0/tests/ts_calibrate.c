...
int main()
{
	...
	cal_fd = open("/mnt/nand1-1/pointercal", O_RDONLY);
	if (cal_fd != -1)
	{
		printf("Touchscreen calibration data already exist.  No need calibration.\n");
		close(cal_fd);
		return 0;
	}
...
// Read a touchscreen event to clear the buffer
	...
	if (perform_calibration (&cal)) {
		...
		if ((calfile = getenv("TSLIB_CALIBFILE")) != NULL) {
			...
			cal_fd = open ("/mnt/nand1-1/pointercal", O_CREAT | O_RDWR);
		}
